...
- `pnpm test:e2e` — Playwright(DC-71). `playwright.config.ts`의 webServer가 **프로덕션 빌드 후 3101 포트**로 띄운다(dev는 on-demand 컴파일로 타임아웃). 이미 띄운 서버를 쓰려면 `E2E_BASE_URL`을 주면 webServer를 건너뛴다. `e2e/global-setup.ts`가 `SUPABASE_SERVICE_ROLE_KEY`로 **E2E 전용 계정을 멱등 생성**하므로 시드 운영자 계정·비밀번호를 리포에 두지 않는다(`E2E_EMAIL`/`E2E_PASSWORD`로 덮어쓸 수 있다). 화면 문구는 하드코딩하지 말고 `messages/ko.json`에서 읽을 것.
