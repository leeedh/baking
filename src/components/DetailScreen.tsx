...
export default function DetailScreen({ classId }: DetailScreenProps) {
  ...
  return (
    <div
      id="detail-screen"
      className="bg-[#FAF4EA] py-8 px-4 sm:px-8 max-w-7xl mx-auto pb-28 lg:pb-8"
    >
          ...
          <div className="flex border-b border-[#EFE8DC] bg-white rounded-t-xl p-1 mt-6">
            <button
              ...
              className={`flex-1 py-3 text-xs sm:text-sm font-semibold rounded-lg transition-colors cursor-pointer ${
                activeTab === 'intro'
                  ? 'bg-[#FAF4EA] text-[#B65538] shadow-sm'
                  : 'text-[#5F4E43]/70 hover:text-[#5F4E43]'
              ...
              <div className="space-y-6 text-sm text-[#2A211B] leading-relaxed font-light">
                <div>
                  <h3 className="font-serif text-lg font-bold text-[#B1863C] mb-2 flex items-center gap-2">
                    <span className="w-1.5 h-4 bg-[#B1863C] rounded-full inline-block" />
                    프리미엄 세션 소개
                  </h3>
                  <p className="text-[#5F4E43] font-light leading-relaxed">{cls.description}</p>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                  <div className="p-4 bg-[#FAF4EA]/40 rounded-xl border border-[#EFE8DC]">
                    <h4 className="font-bold text-xs text-[#B65538] uppercase mb-2">
                      이 클래스에서 학습하는 레시피 구성
                    </h4>
                  ...
                  <div className="p-4 bg-[#FAF4EA]/40 rounded-xl border border-[#EFE8DC]">
                    <h4 className="font-bold text-xs text-[#B0863C] uppercase mb-2">
                      수강생에게 제공되는 다운로드 혜택
                    </h4>
                    <ul className="space-y-1.5 text-xs text-[#5F4E43]">
                ...
                <div className="pt-4">
                  <h3 className="font-serif text-lg font-bold text-[#B65538] mb-2">
                    수강 전 확인사항
                  </h3>
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  {curriculum.map((chapter) => (
                    <div
                      key={chapter.id}
                      className="border border-[#EFE8DC] rounded-xl overflow-hidden shadow-sm"
                    >
                      <div className="bg-[#FAF4EA]/50 p-4 border-b border-[#EFE8DC] flex justify-between items-center">
                        ...
                        <span className="text-[11px] text-[#B0863C] font-semibold">
                        ...
                        {chapter.lessons.map((lesson) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="flex items-center gap-3">
                                <span className="text-xs text-[#5F4E43] font-mono">
              ...
              <div className="space-y-6">
                <div className="flex items-center gap-4 bg-[#FAF4EA] p-4 rounded-xl border border-[#EFE8DC] mb-4">
                  <div className="text-center p-2">
                    <span className="block text-3xl font-serif font-extrabold text-[#B65538]">
                      {cls.rating}
                    </span>
                    <span className="text-[10px] text-[#5F4E43]">수강생 평점</span>
                  </div>
                  <div className="h-10 w-px bg-[#EFE8DC]" />
                ...
                <div className="space-y-4 divide-y divide-[#EFE8DC]">
                  {reviews.length === 0 ? (
                    <div className="text-center py-8 text-xs text-[#5F4E43]/60">
                      ...
                      <div key={rev.id} className="pt-4 space-y-2 first:pt-0">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            ...
                            />
                            <div>
                              <span className="text-xs font-bold text-[#2A211B]">
                                {rev.userName}
                              </span>
                              <div className="flex items-center text-[#B0863C] transform scale-90 -translate-x-1.5">
                                {[1, 2, 3, 4, 5].map((starPosition) => (
                                  <Star
                                    ...
                                    size={10}
                                    className={
                                      starPosition <= rev.rating ? 'fill-[#B0863C]' : 'opacity-20'
                                    }
                                  />
                                ))}
                              </div>
                          ...
                          <span className="text-[10px] text-[#5F4E43]/60 font-mono">
                            {rev.date}
                          </span>
                        </div>
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-[#EFE8DC] p-3 pb-[max(12px,env(safe-area-inset-bottom))] shadow-[0_-8px_20px_rgba(42,33,27,0.08)] flex items-center justify-between gap-3 px-4">
        <div className="flex flex-col">
          <span className="text-[9px] font-bold text-[#B65538] uppercase tracking-wider">
