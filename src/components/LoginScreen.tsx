...
import { Mail, Compass, Star, Lock, Eye, EyeOff, Check, Chrome, Sparkles } from 'lucide-react';
...
export default function LoginScreen({ onLoginSuccess, onNavigateToCatalog }: LoginScreenProps) {
  ...
  return (
    <div id="login-screen" className="flex items-center justify-center min-h-[80vh] px-4 py-12 bg-[#FAF4EA]">
      <div id="login-card" className="w-full max-w-md bg-white rounded-2xl shadow-xl border border-[#EFE8DC] overflow-hidden transform hover:scale-[1.01] transition-transform duration-300">
        ...
        <div className="p-6 sm:p-10">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {!isLoginTab && (
              <div>
                <label className="block text-xs font-semibold text-[#2A211B] uppercase tracking-wide mb-1.5">이름 (실명)</label>
            ...
            {/* Email */}
            <div>
              <label className="block text-xs font-semibold text-[#2A211B] uppercase tracking-wide mb-1.5">이메일 주소</label>
              <div className="relative">
            ...
            <div>
              <div className="flex justify-between items-center mb-1.5">
                <label className="block text-xs font-semibold text-[#2A211B] uppercase tracking-wide">비밀번호</label>
                {isLoginTab && (
                  <a href="#find-pw" onClick={(e) => {e.preventDefault(); setErrorMsg("이메일로 임시 비밀번호 전송 기능을 구현할 수 있습니다.");}} className="text-[10px] text-[#B0863C] hover:underline font-medium">비밀번호를 잊으셨나요?</a>
                )}
              </div>
              <div className="relative">
                <span className="absolute inset-y-0 left-0 pl-3 flex items-center text-[#5F4E43]/50">
          ...
          <div className="relative my-6">
            <div className="absolute inset-0 flex items-center">
              <div className="w-full border-t border-[#EFE8DC]"></div>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            >
              <svg className="w-4 h-4" viewBox="0 0 24 24">
                <path fill="#4285F4" d="M23.745 12.27c0-.7-.06-1.4-.19-2.07H12v3.9h6.69c-.29 1.5-.143 2.78-1.566 3.73l2.451 1.91c1.433-1.32 2.26-3.26 2.26-5.56z"/>
                <path fill="#34A853" d="M12 24c3.24 0 5.95-1.08 7.93-2.91l-3.83-2.97c-1.08.72-2.45 1.16-4.1 1.16-3.16 0-5.83-2.14-6.79-5.01H1.28v3.09C3.25 21.3 7.31 24 12 24z"/>
                <path fill="#FBBC05" d="M5.21 14.27a7.08 7.08 0 010-4.54V6.64H1.28a11.936 11.936 0 000 10.72l3.93-3.09z"/>
                <path fill="#EA4335" d="M12 4.75c1.77 0 3.35.61 4.6 1.8l3.44-3.44C17.93 1.19 15.24 0 12 0 7.31 0 3.25 2.7 1.28 6.64l3.93 3.09c.96-2.87 3.63-5.01 6.79-5.01z"/>
              </svg>
              Google 로그인
            </button>
          ...
          <p className="text-[10px] text-center text-[#5F4E43]/60 mt-8 leading-normal">
            가입 시 Atelier Crème의 <a href="#terms" className="underline hover:text-[#B65538]">서비스 이용약관</a> 및 <a href="#privacy" className="underline hover:text-[#B65538]">개인정보 처리방침</a>에 동의하게 됩니다. 해외 수강생 서비스 및 중문 번역 기능은 로그인 후 프로필에서 설정 가능합니다.
          </p>
