...
import { BookOpen, Check, Award, ShoppingBag, Download, ArrowRight, Heart, Star, Sparkles, MessageSquare } from 'lucide-react';
...
export default function BooksScreen() {
  const books: BookItem[] = [
    {
      ...
      originalPrice: 45000,
      discountPercent: 28,
      thumbnail: 'https://images.unsplash.com/photo-1544947950-fa07a98d237f?auto=format&fit=crop&q=80&w=600', // Vertical book-looking orientation
      ...
      description: '단순히 ‘밀가루와 계란을 섞어 굽는다’식의 기초 설명서를 거부합니다. 밀가루 속 전분의 호화가 일어나는 임계 온도, 가나슈 필링이 꼬끄 속에서 수분을 흡수하기 가장 좋은 유성 비율 등, 10년 노하우를 시각적 차트와 수학 공식으로 낱낱이 도해해 낸 국내 유일한 프로용 제과 에디토리얼 가이드북입니다.',
      benefits: [
        '소장 즉시 1:1 상담 질문 게시판 무제한 이용권',
        '전체 제품 상업 배합 비율 마스터용 오토 엑셀 캘큘레이터 다운로드',
        '한컴/PDF 포맷 모바일/PC 프리미엄 전용 리더 지원'
      ],
      pdfSpecInfo: '구매 즉시 내 강의실 자료판 및 가입 이메일로 암호화 PDF 배송 및 영구 다운로드 제공'
    },
    {
      ...
      description: '온라인 가이드 영상만으로는 아쉬웠던 프랑스 클래식 제과의 핵심 골격을 담았습니다. 머랭의 밀도가 꼬끄의 빳빳함과 겉 식감에 어떤 영향을 끼치는지, 실제 믹싱 기어 수치와 접사 사진으로 상세히 담았습니다. Sugar Lane식 계량 정교화와 Happy Happy Academy식의 탁월한 비주얼 철학이 만난 도서입니다.',
      benefits: [
  ...
  const handlePurchaseBook = (bookId: string) => {
    ...
    setTimeout(() => {
      ...
      alert(`🎉 [${selectedBook.title}] 도서 및 디지털 비밀 배합 배포 패키지 구매가 정상 승인되었습니다! 가입하신 메일로 원가계산 전용 스마트 엑셀 프로그램과 레시피 PDF 교재가 즉시 직동 교부되었습니다.`);
    }, 1200);
  };

  const handleDownloadMaterials = (bookTitle: string) => {
    alert(`📥 [${bookTitle}] 수강생 전용 상업용 오토 엑셀 배합 파일 및 교안 PDF 파일 다운 로드를 시작합니다. (Atelier-Creme-Secret-Recipe.zip)`);
  };

  return (
    <div id="books-screen" className="bg-[#FAF4EA] min-h-screen text-[#2A211B] font-sans selection:bg-[#B65538]/20 selection:text-[#B65538]">
      ...
      <section className="pt-12 pb-16 px-6 sm:px-12 max-w-7xl mx-auto border-b border-[#EFE8DC]/80">
        <div className="text-center max-w-3xl mx-auto space-y-4">
          ...
          <h1 className="font-serif text-3xl sm:text-5xl font-bold text-[#2A211B] leading-tight keep-all break-keep">
            오너 파티시에의 비공개 <span className="font-serif italic text-[#B65538]">비밀 레시피 북</span>
          </h1>
          <p className="text-sm text-[#5F4E43] font-light leading-relaxed max-w-2xl mx-auto keep-all break-keep">
            매일 아침 부지런하게 구워지며 소리 없이 전석 매진을 그려내는 한남동 크렘 쇼룸의 오리지널 레시피 설계 기획서. 동영상 강의의 깊이를 더욱 예리하게 보강해주는 실전 교안 도서를 평생 영구 소장하시고, 매장 매출의 고품격 돌파구를 만드세요.
          </p>
        </div>
      </section>
      ...
      <section className="py-12 px-6 sm:px-12 max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <div className="lg:col-span-4 space-y-6">
          <h3 className="font-serif text-xs font-bold text-[#B0863C] tracking-wider uppercase mb-2">Select Edition</h3>
              ...
              return (
                ...
                >
                  ...
                  <div className="w-16 sm:w-20 aspect-[3/4] bg-[#2A211B] rounded overflow-hidden shadow-md shrink-0 relative">
                    ...
                    />
                    <div className="absolute top-1 left-1.5 bg-[#B65538] text-[7px] text-white font-extrabold px-1 rounded">BEST</div>
                  </div>

                  <div className="flex-1 flex flex-col justify-between">
                    <div>
                      <span className="text-[9.5px] text-[#B0863C] font-semibold tracking-wider block mb-1 uppercase">{b.edition}</span>
                    ...
                    <div className="flex items-center justify-between pt-2">
                      <span className="text-xs font-bold text-[#B65538]">₩{b.price.toLocaleString()}원</span>
          ...
          <div className="p-6 bg-[#2A211B] text-[#FAF4EA] rounded-2xl border border-[#B0863C]/30 space-y-4">
            <span className="text-[9px] bg-[#B65538] px-2 py-0.5 rounded font-extrabold tracking-widest uppercase">PROMOTION</span>
            ...
            <p className="text-[11px] text-white/70 leading-relaxed font-light">
              마카롱 바이블과 시그니처 구움과자 책을 동시에 소장하시는 분들께 1:1 민소희 마스터 이메일 오븐 분석권 3회를 더불어 무상 전개해 드립니다.
            </p>
            <div className="pt-2">
              <button 
                onClick={() => {
        ...
        <div className="lg:col-span-8 bg-white border border-[#EFE8DC] rounded-3xl p-6 sm:p-10 space-y-8 shadow-sm">
          ...
          <div className="space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-[#EFE8DC] pb-5">
              <div>
                <span className="text-[11px] text-[#B1863C] font-bold block mb-1">SELECTED MANUAL</span>
                <span className="text-[10px] bg-[#FAF4EA] border border-[#EFE8DC] text-[#5F4E43] px-2.5 py-1 rounded font-medium">{selectedBook.edition}</span>
                <h2 className="font-serif text-2xl sm:text-3xl font-bold text-[#2A211B] mt-2 leading-tight">
                  {selectedBook.title}
                </h2>
                <p className="text-xs sm:text-sm text-[#B65538] font-medium font-serif mt-1">{selectedBook.subTitle}</p>
              </div>

              {/* Price & Discount Indicator */}
              <div className="text-right shrink-0 bg-[#FAF4EA] p-4 rounded-xl border border-[#EFE8DC]">
                <span className="text-xs text-stone-400 line-through block">₩{selectedBook.originalPrice.toLocaleString()}원</span>
                <div className="flex items-center gap-1.5 justify-end">
                  <span className="text-[#B65538] text-xs font-black">{selectedBook.discountPercent}% OFF</span>
                  <span className="text-xl font-bold text-[#2A211B]">₩{selectedBook.price.toLocaleString()}</span>
                </div>
                <span className="text-[10px] text-stone-400 font-light block mt-0.5">배송비 전액 0원 무료 지원</span>
              </div>
            </div>
          ...
          <div className="space-y-3 bg-[#FAF4EA]/40 p-6 rounded-2xl border border-[#EFE8DC]/70">
            ...
            <div className="divide-y divide-[#EFE8DC] text-xs">
              {selectedBook.chapters.map((ch, idx) => (
                <div key={idx} className="py-3 font-medium text-[#5F4E43] flex items-start gap-2.5">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {selectedBook.benefits.map((ben, idx) => (
                <div key={idx} className="p-4 rounded-xl border border-[#EFE8DC] bg-white space-y-2 flex flex-col justify-between">
          ...
          <div className="p-4 bg-yellow-50/50 border border-yellow-100 rounded-xl">
            ...
            <p className="text-[10px] text-[#5F4E43] mt-0.5 font-light leading-relaxed">
              {selectedBook.pdfSpecInfo}. 결제 완료 후 12시간 이내에 택배 발송 번호가 문자로 동기화 연동됩니다.
            </p>
          </div>
          ...
          <div className="pt-4 border-t border-[#EFE8DC] flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div className="flex gap-2 w-full sm:w-auto">
              {isBookPurchased[selectedBook.id] ? (
                ...
                >
                  <ShoppingBag size={14} />
                  <span>{isPurchasing ? '보안 인증 및 결제 승인 중...' : '도서 즉시 평생 소장 라이선스 획득'}</span>
                </button>
              )}
            </div>
      ...
      <section className="py-12 bg-white/50 border-t border-[#EFE8DC]/80">
        <div className="max-w-7xl mx-auto px-6 sm:px-12 grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
          ...
          <div className="space-y-1">
            <h4 className="font-serif text-sm font-bold text-[#2A211B]">평생 스마트 엑셀 무료 패원</h4>
            <p className="text-[10px] text-[#5F4E43] font-light">사재 원재료 시세 연어 자동 수혈 기능</p>
          </div>
          <div className="space-y-1">
            <h4 className="font-serif text-sm font-bold text-[#2A211B]">스페셜 전용 자료실 개설</h4>
            <p className="text-[10px] text-[#5F4E43] font-light">민소희 마스터 독점 첨삭 PDF 평생공유</p>
          </div>
          <div className="space-y-1">
            <h4 className="font-serif text-sm font-bold text-[#2A211B]">개정 증보판 무제한 갱신</h4>
            <p className="text-[10px] text-[#5F4E43] font-light">배송비용 하나로 모바일 리커버 평생갱신</p>
          </div>
        </div>
      </section>
