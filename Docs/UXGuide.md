...
테라코타 hover 5종·골드 hover 4종과 오타 `#B1863C`는 더 이상 존재하지 않는다.
...
> DC-57 범위에서 제외했고, 정리하려면 별도 티켓이 필요하다.
> ℹ️ 다크 테마·`success/danger/info` 시맨틱 토큰은 **미구현**(성공 톤은 `Badge`/`Toast`가 emerald로 공유).
