...
> ⚠️ **본 문서는 현재 구현된 코드(`src/`)를 기준으로 작성됨.** 백엔드(Supabase/Mux/TossPayments), 라우팅(Next.js `[locale]`), 다크 테마 등은 현재 구현에 존재하지 않으며, 필요 시 §7.3의 "향후 과제"로만 표기한다.
...
- 테라코타 hover: `#9C3F24` · `#A0452C` · `#9E3E23` · `#A14328` (혼용, 통일 필요)
- 골드 hover: `#9A7230`
- 상태색: 완료 `emerald-600/700`, 정보 `sky-700`, 경고 토스트 `amber-50/200/900`

> ⚠️ **개선 포인트**: 테라코타 hover 값이 파일마다 제각각이다. 단일 `brand-deep` 토큰으로 통일 권장.
...
- **구성**: 로그인/회원가입 **탭 전환**, 이메일·비밀번호, 소셜 로그인(Google/Apple, 모바일 1열), 약관.
...
- **컴포넌트**: `BooksScreen`.
- **구성**: 레시피 도서 카드(표지·에디션·페이지·챕터 목록·혜택·가격/할인), PDF·실물 배송 안내, 구매 CTA.
